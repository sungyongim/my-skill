...
### Step 0: 전날 미작성 회의록 정리 (사전 청소)

morning 모드 진입 시 **가장 먼저** 실행한다. 어제 자동 생성된 회의록 중 사용자가 본문을 채우지 않고 템플릿 그대로 남아 있는 파일은 삭제한다.

#### 0-1. 대상 파일

- 디렉토리: `/Users/sy.im/Documents/내창고/업무/회의록/`
- 패턴: `{어제 YYYY-MM-DD}_*.md` (어제 = 대상 날짜 − 1일)
- 그 이전 날짜의 파일은 건드리지 않는다 (사용자가 보관 중일 수 있음).

#### 0-2. 템플릿 판별 규칙

각 후보 파일을 `Read`로 읽어 **본문 5개 섹션이 모두 비어 있는지** 확인한다:

판별 기준 — 아래 5개 헤더(`## 안건`, `## 논의 내용`, `## 결정 사항`, `## 액션 아이템`, `## 다음 단계 / 후속 회의`) 사이의 본문 라인이 **다음 패턴들로만** 구성된 경우 템플릿으로 간주한다:
- 빈 줄
- `-` 단독 (placeholder dash)
- `- [ ] **담당자** — 내용 (마감)` (placeholder 액션 아이템)

위 패턴 외에 한 줄이라도 의미 있는 텍스트가 있으면 **사용자가 작성한 회의록**으로 보고 **삭제하지 않는다**.

> frontmatter(`---` 사이의 metadata)는 판별 대상이 아니다 — 비어 있어도 항상 채워져 있음.

#### 0-3. 삭제 실행

- 템플릿으로 판별된 파일은 `Bash`의 `rm` 명령으로 삭제한다.
- 삭제 전 후보 파일 목록을 출력하고, 각각 판별 결과(템플릿/사용자작성)를 명시한다.
- 안전: 한 번에 하나씩 또는 일괄(`rm file1 file2 ...`) 가능.

#### 0-4. 결과 보고

Step 3 출력 markdown의 **"검색 출처" 섹션 위쪽**에 다음 섹션을 추가한다:

```markdown
## 정리된 회의록 (어제분 미작성 템플릿)

- `2026-04-28_1000_데일리-지표-미팅.md` (삭제) — 본문 빈 템플릿
- `2026-04-28_1100_핀테크-기획-위클리.md` (보존) — 사용자 작성됨
- (해당 파일이 0건이면 "어제 미작성 회의록 없음" 표기)
```

evening 모드에서는 본 단계를 실행하지 않는다 (그날 회의록은 사용자가 늦게라도 작성할 수 있으므로 보존).
